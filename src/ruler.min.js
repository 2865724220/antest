...
(function (window) {
    ...
    var Ruler = function (id, paraObj) {
        ...
        that.maxNum = paraObj.maxNum || 300.0; //最大数值
        that.minNum = paraObj.minNum || 30.0; //最小数字
        that.cellNum = paraObj.cellNum || 10; //一个大刻度的数值区间
        that.minNum = parseInt(that.minNum / that.cellNum) * that.cellNum;
        that.record = [];
        if ((that.maxNum - that.minNum) % that.cellNum !== 0) {
            that.maxNum = that.minNum + that.cellNum * parseInt((that.maxNum - that.minNum) / that.cellNum + 1);
        }
        that.name = paraObj.name;
        that.unit = paraObj.unit;
        that.initNum = paraObj.initNum;
        that.nowData = that.initNum || that.minNum || 30.0;
        that.decimalWei = paraObj.decimalWei;
        //做出标尺的html结构
        that.rulerStructure(id); //传入id值
        //画出标尺的静态
        that.drawRuler();
        //添加滑动标尺功能
        that.moveRuler();
    ...
    Ruler.prototype.rulerStructure = function (id) {
        ...
        //标题中间的文字和+ -
        parameterNode.innerHTML = "<div id='rulerTitle' style='color:#fff;float:left; font-size:24px; line-height:28px;display: none'>+</div>"+
            "<div id='ruler-num' style=' color:#FB8801; font-size:40px;display:inline-block;font-family: DIN-Regular;vertical-align: middle;line-height:40px;'>30</div>"+
            "<div style='display: inline-block;vertical-align: middle;font-size:20px;color:#FB8801;padding:0 2px;'>岁</div>"+
            "<div id='ruler-unit' style='color:#fff;line-height: 28px;font-size: 24px;float:right;display: none;'>bpm</div>";
        //声明结果对象
    ...
    Ruler.prototype.drawPin = function (obj) {
        ...
        pinCanvas.strokeStyle = "#FB8801"; //中间标线颜色
